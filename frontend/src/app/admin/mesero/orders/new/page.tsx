...
// Fallback toast hook: some environments may not have the shared use-toast module.
// This provides a minimal replacement so the page still works.
const useToast = () => ({
  toast: ({ title, description, variant }: { title?: string; description?: string; variant?: string }) => {
    try {
      // Prefer console logging in dev, but also show a simple alert for visibility.
      // Keep this minimal to avoid importing missing UI modules.
...
export default function NewOrderPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: 'Éxito', description: 'Pedido enviado al bartender' })
      router.push('/mesero')
    } catch (error) {
              ...
              <CardContent className="p-4">
                  ...
                  {cart.map(item => (
                    <div key={item.receta_id} className="flex items-center justify-between border-b pb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <Button size="sm" variant="ghost" onClick={() => updateQuantity(item.receta_id, item.cantidad - 1)}>
                        ...
                        <Button size="sm" variant="ghost" className="text-red-500" onClick={() => removeFromCart(item.receta_id)}>
                          <Trash2 className="h-3 w-3" />
                        </Button>
                      </div>
                    </div>
                  ))}
                  {cart.length === 0 && <p className="text-gray-400 text-sm">No hay productos</p>}
                </div>
                <div className="mt-4 pt-4 border-t">
                  ...
                  <div className="mt-2">
                    ...
                    <Input id="mesa" placeholder="Número de mesa" value={mesa} onChange={(e) => setMesa(e.target.value)} />
                  </div>
                  <Button className="w-full mt-4 bg-orange-600 hover:bg-orange-700" onClick={handleSubmit} disabled={loading || cart.length === 0}>
