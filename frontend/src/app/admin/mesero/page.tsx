...
import { Plus, Coffee, List, LogOut, Menu, Bell, Search } from 'lucide-react'
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3">
              ...
              <Avatar className="cursor-pointer">
                <AvatarFallback className="bg-orange-600 text-white">MS</AvatarFallback>
              </Avatar>
      ...
      <main className="px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">¡Hola, {user?.email?.split('@')[0] || 'Mesero'}!</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent>
              <p className="text-3xl font-bold">{orders.filter(o => o.mesero_id === user?.id).length}</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
                ...
                <div className="space-y-3">
                  {orders.slice(0, 5).map((order) => (
                    <div key={order.id} className="flex items-center justify-between border-b pb-2">
                      ...
                      <Badge className={getStatusColor(order.estado)}>{order.estado.toUpperCase()}</Badge>
                    </div>
                  ))}
                </div>
              )}
